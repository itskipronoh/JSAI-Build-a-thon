...
export class ChatInterface extends LitElement {
 ...
 async _sendMessage() {
  ...
  try {
    ...
    const assistantMessage = {
      role: 'assistant',
      content: aiResponse.reply, // ✅ This is the actual string
      sources: aiResponse.sources // ✅ Store sources separately for rendering
    };
 ...
 async _apiCall(message) {
  ...
  // async _mockAiCall(userQuery) {
  //   return new Promise(resolve => {
  //     setTimeout(() => {
  //       resolve(`AI response to: ${userQuery}`);
  //     }, 1000);
  //   });
  // }
